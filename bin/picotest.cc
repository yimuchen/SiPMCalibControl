...
int32_t
fopen_s( FILE** a, const char* b, const char* c )
{
  FILE* fp = fopen( b, c );
  *a = fp;
  return ( fp > 0 ) ? 0 : -1;
...
****************************************************************************/
void PREF4
CallBackStreaming( int16_t handle,
                   int32_t noOfSamples,
                   uint32_t startIndex,//
...
****************************************************************************/
void PREF4
CallBackBlock( int16_t handle, PICO_STATUS status, void* pParameter )
{
  // flag to say done reading data
  g_ready = TRUE;
}
...
****************************************************************************/
int16_t
mv_to_adc( int16_t mv, int16_t ch )
{
...
****************************************************************************/
void
BlockDataHandler( UNIT_MODEL* unit, char* text, int32_t offset )
{
  int32_t i, j;
...
****************************************************************************/
void
CollectBlockImmediate( UNIT_MODEL* unit )
{
...
void
CollectBlockEts( UNIT_MODEL* unit )
{
  PICO_STATUS status;
...
****************************************************************************/
void
CollectBlockTriggered( UNIT_MODEL* unit )
{
  int16_t triggerVoltage = mv_to_adc( 100,
    unit->channelSettings[PS5000_CHANNEL_A].range );  // ChannelInfo stores ADC counts
  struct tTriggerChannelProperties sourceDetails =
...
****************************************************************************/
void
CollectBlockLogicTriggered( UNIT_MODEL* unit )
{
  int16_t triggerVoltage = mv_to_adc( 100,
    unit->channelSettings[PS5000_CHANNEL_A].range );  // ChannelInfo stores ADC counts
...
****************************************************************************/
void
CollectRapidBlock( UNIT_MODEL* unit )
{
  int32_t timeInterval;
  int32_t maxSamples;
  int32_t sampleCount = BUFFER_SIZE;
 ...
 ***************************************************************************/
void
SetSignalGenerator( UNIT_MODEL unit )
{
  PICO_STATUS status;
  int16_t waveform;
  int32_t frequency;
  ...
  if( ch == 'X' ){// If we're going to turn off siggen
    ...
    if( fopen_s( &fp, fileName, "r" ) == 0 ){// Having opened file, read in data - one number per line (at most 8192 lines), with values in (0..4095)
 ...
 ***************************************************************************/
void
StreamDataHandler( UNIT_MODEL* unit, uint32_t preTrigger )
{
  int32_t i, j;
 ...
 ***************************************************************************/
void
CollectStreamingImmediate( UNIT_MODEL* unit )
{
  struct tPwq pulseWidth;
  struct tTriggerDirections directions;
 ...
 ***************************************************************************/
void
CollectStreamingTriggered( UNIT_MODEL* unit )
{
  int16_t triggerVoltage = mv_to_adc( 100,
    unit->channelSettings[PS5000_CHANNEL_A].range );  // ChannelInfo stores ADC counts
