 ...
 * Notice
 * that the system lock will automatically be removed when the corresponding
 * file
 ...
 * PWM SYSFS INTERFACE
 *
 * The example of accessing the sysfs for the PWM interface can be found
 * here[1],
 ...
 * interface is initialized, the readout is continuously flushed to a buffer in
 * a
 ...
 *
 * This setups up the default readout and and pin numbers and other modes.
 * Notice that the interfaces will not initialized until explicitly told to do
 * so by the upper python function. This is so that the upper python process can
 * choose the order or initialization in case certain interfaces doesn't exist.
 *
 *******************************************************************************/
GPIO::GPIO() :
