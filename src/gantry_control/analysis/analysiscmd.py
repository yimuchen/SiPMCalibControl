...
class lowlightcollect(cmdbase.singlexycmd, cmdbase.readoutcmd, cmdbase.rootfilecmd):
    ...
    def run(self, args):
        ...
        for _ in self.start_pbar(range(args.nparts)):
            ...
            self.fillroot(
                {"readout": readout}, {"readout": "var * float64"}, det_id=args.detid
            )
...
class timescan(cmdbase.readoutcmd, cmdbase.rootfilecmd):
    ...
    def add_args(self):
        self.parser.add_argument(
            "--nslice", type=int, default=30, help="total number of measurement to take"
        )
        self.parser.add_argument(
            "--interval",
            type=float,
    ...
    def run(self, args):
        ...
        for it in self.start_pbar(args.nslice):
            ...
            self.fillroot(
                {"lumival": lumival, "uncval": uncval, "S2": s2, "S4": s4},
                time=timestamp,
                det_id=-100,
            )
            self.pbar_data(
                Lumi=f"{lumival:.2f}+-{uncval:.2f}", Sharp=f"({s2:.1f}, {s4:.1f})"
            )
  ...
  """
    ...
    def run(self, args):
        ...
        for xval, yval in self.start_pbar(zip(args.x, args.y)):
            ...
            self.fillroot(
                {"center x": center.x, "center y": center.y}, det_id=args.detid
            )
            self.pbar_data(
                center=f"({center.x:.0f}, {center.y:.0f})",
                sharp=f"({center.s2:1f}, {center.s4:.1f})",
            )
        fitx, covar_x = curve_fit(
            visualhscan.model, np.vstack((gantry_x, gantry_y)), reco_x
        )
        fity, covar_y = curve_fit(
            visualhscan.model, np.vstack((gantry_x, gantry_y)), reco_y
        )
        ...
        if self.board.visM_hasz(args.detid, args.scanz):
            ...
            args.calibdet = next(
                iter(
                    [
                        x
                        for x in self.board.calib_dets()
    ...
    def run(self, args):
        ...
        for _ in range(16):
          ...
          with command visualdetshow'"""
            ...
            if np.linalg.norm(motionxy) < 0.1:
                break
            self.move_gantry(
                self.gcoder.opx + motionxy[0],
                self.gcoder.opy + motionxy[1],
                self.gcoder.opz,
            )
        ...
        self.printmsg(
            ...
            "Det FOV position: x={2:.1f} y={3:.1f}".format(
                self.gcoder.opx, self.gcoder.opy, center.x, center.y
            )
        )
        self.printmsg(
        ...
        if not self.board.vis_coord_hasz(detid, self.gcoder.opz) or args.overwrite:
            self.board.add_vis_coord(
                detid, self.gcoder.opz, [self.gcoder.opx, self.gcoder.opy]
            )
            self.conditions.update_gantry_and_sipm_conditions(
        ...
        if not self.board.lumi_coord_hasz(detid, self.gcoder.opz):
            ...
            for calibdet in self.board.calib_dets():
                ...
                if self.board.vis_coord_hasz(calibdet, currentz) and (
                    self.board.get_latest_entry(args.detid, "halign") is not None
                ):
                    closestz = self.board.get_closest_calib_z(detid, "halign", currentz)
                    deltax = (
                        self.board.get_vis_coord(calibdet, currentz)[0]
                        - self.board.get_lumi_coord(calibdet, closestz)[0]
                    )
                    deltay = (
                        self.board.get_vis_coord(calibdet, currentz)[1]
                        - self.board.get_lumi_coord(calibdet, closestz)[2]
                    )
                if deltax != None and deltay != None:
                    self.printmsg(
                        "Estimated Lumi center: x={0} y={1}".format(
...
class visualmaxsharp(cmdbase.singlexycmd, cmdbase.zscancmd, visualmeta):
  ...
  """
    ...
    def run(self, args):
        ...
        model = (
            visualmaxsharp.quad_model()
            if args.fitmodel == "quad"
            else visualmaxsharp.gauss_model()
            if args.fitmodel == "gauss"
            else visualmaxsharp.cubic_model()
        )
        ...
        fit, cov = curve_fit(
            ...
            # Initial guess
            bounds=(
                [np.min(args.zlist), *model.bounds[0]],
...
class visualzscan(
    cmdbase.singlexycmd, cmdbase.zscancmd, visualmeta, cmdbase.rootfilecmd
):
    """
    ...
    def run(self, args):
        for z in self.start_pbar(args.zlist):
            ...
            self.pbar_data(
                sharpness=f"({center.s2:.1f}, {center.s4:.1f})",
